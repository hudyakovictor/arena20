# T042 · Reveal-флоу (факт→последствие→качество)
...
- **Зависимости:** T041, T021
- **CEO-гейт:** нет
- **Обновлено:** 2026-09-08

## Цель
После решения: раскрытие истории (T21) → последствие действия → оценка качества 0–100.

## Критерии приёмки
- [ ] 3 слоя: рыночный факт / последствие / качество
- [ ] Качество не зависит от направления цены
- [ ] Контролируемый исторический убыток может получить максимум

## Тест-план
- Playwright: reveal показывает 3 слоя
- Vitest: качество не коррелирует с PnL
...
## Команды статуса
```bash
python3 github_project/_meta/generate.py set-status T042 in_progress --agent <id> --note "что начато"
python3 github_project/_meta/generate.py set-status T042 done --agent <id> --note "тесты и результат"
```
